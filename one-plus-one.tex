...
%% with new filenames distinct from one-plus-one.tex.
%%
...
%%
%%
\documentclass[acmsmall,review,screen,nonacm,authorversion]{acmart}
%%
...
\lstdefinelanguage{colang}{
    keywords=[1]{let, var, effect, perform, data, context, fun, where, if, else, throw, try, catch, local, free, op, handle, resume, class, interface, override, init},
    sensitive=true,
    comment=[l]{//},
    morecomment=[s]{/*}{*/},
    morestring=[b]",
...
Given that effect systems operate at the type level, it is natural to employ type-based techniques for escape analysis as well~\cite{xie2022first}. % todo better understand connection with dataflow algorithms, since they are semantics too
...
% Note: it is still not possible to store regional in mutable state - for good.
...
This approach has led to the realization that effect systems are fundamentally concerned with the management of free variables, and has enabled the discovery of contextual polymorphism~\cite{brachthauser2020effects, brachthauser2022effects}.
...
Furthermore, we compute lifetimes of type variables by approximating them with the lifetimes of their bounds, whereas capture types employ more intricate boxing mechanisms, which remain unsettled at the time of writing~\cite{xu2023formalizing, xu2025s}.
...
% todo chapter about escape analysis
% todo compare with effect systems and other analyses
