...
В работе рассматривается исчисление Core-Delta, в котором времена жизни образуют двухточечную решётку с операцией минимума (побеждает более короткое, более ограничительное время жизни), составные данные скрывают времена жизни полей экзистенциально и восстанавливают их при сопоставлении с образцом, а алгебраические обработчики эффектов передаются как значения-возможности.
...
Проверено, что ни одна ключевая теорема не зависит от недоказанных допущений, так что доверенная база сводится к ядру Rocq и его стандартной библиотеке.
Механизация переиспользуема как такая спецификация; корректность размещения в памяти остаётся отдельным аргументом.}
...
We study Core-Delta, a calculus in which lifetimes form a two-point lattice with a minimum operation (the shorter, more restrictive lifetime wins), compound data hide field lifetimes existentially and recover them on pattern matching, and algebraic effect handlers are passed as capability values.
...
Every capstone theorem is verified to depend on no unproved assumptions, so the trusted base is the Rocq kernel and its standard library.
The mechanization is reusable as such a specification; memory placement itself remains a separate argument.}
...
Mechanization also returns more than confidence: the proofs force every rule into executable form, so the artifact doubles as a reference implementation---reflected deciders for the side conditions and a certified evaluator---that a compiler can be tested against.
...
A compound value's lifetime is the minimum (written \lstinline[language=colang]|+|) of its components' lifetimes, and destructuring recovers component lifetimes from the container's; that paper develops this value discipline in detail, together with lifetime polymorphism and bounded quantification over tracked types.
...
\begin{lstlisting}[language=colang,label={lst:reader},caption={A reader handler in surface notation. Each \texttt{perform} suspends to the handler, which resumes with \texttt{env}; the capability \texttt{c} is $\local$ and cannot leave the $\keyword{handle}$ block.}]
        ...
        fun withReader(env: Int,
            ...
            handle c: Reader<Int> { ask => resume(env) }
        ...
        fun leak(env: Int): Reader<Int> =
            handle c: Reader<Int> { ask => resume(env) } in c
\end{lstlisting}
...
\begin{lstlisting}[language=colang,label={lst:state},caption={A two-operation state effect. One capability \texttt{st} grants both \texttt{get} and \texttt{put}; the handler threads the state through returned functions.}]
        ...
        fun withState<s <: Any'free, r <: Any'free>(
                body: (State<s>) -local-> r): (s) -local-> r =
            handle st: State<s> {
...
The model must therefore (i) express lifetimes in types without overloading the user with parameters; (ii) confine a runtime capability to its handler; (iii) admit first-class continuations while keeping them out of every escapably typed position; and (iv) have a checkable metatheory suitable as a compiler specification.
...
A capability value has a $\local$ data type, and a resumption is a function with closure lifetime $\local$ (Figure~\ref{fig:handlers}, top): both are therefore barred from every escapably typed position by the function discipline above.
...
In \rname{H-Perform}, $t_{op}$ is the stored clause that the index $op$ selects, and $P$ must be both pure for $m$ and a well-formed evaluation context ($\mathsf{ectx\_wf}$ in the artifact)---purity picks the delimiter, well-formedness the unique call-by-value capture point; $\overline{\beta}$ are the selected operation's type parameters, instantiated at the perform-site arguments $\overline{S}$ before the term substitution; their binders carry the bound $\Any\,\free$, the static counterpart of \rname{T-Perform}'s $\overline{\noloc_\Gamma(S)}$; in the mechanization the captured context $P$ is also shifted across the resumption's new $\lambda$-binder.
...
The operational semantics still mints globally fresh markers in \rname{S-Handle}; freshness is simply not part of the \emph{preserved} invariant, and marker shadowing is harmless because the semantics always addresses the \emph{innermost} occurrence.
...
\emph{Positive examples.} \texttt{Examples.v}/\texttt{ExamplesProofs.v} mechanize the programs of Listings~\ref{lst:reader} and~\ref{lst:state} in monomorphic, inlined form (\texttt{reader\_example}, \texttt{state\_example}) together with exception and optionality handlers, each with a machine-checked typing proof and a reduction trace.
...
On the two-operation \texttt{withState} run of Listing~\ref{lst:state} (\texttt{state\_example} in the artifact), the capstone guarantees---soundness, confinement, and a boundary event---are witnessed directly (\texttt{ExamplesSafety.v}).
