...
В работе рассматривается исчисление Core-Delta, в котором времена жизни образуют двухточечную решётку (free ниже local) с операцией минимума, составные данные скрывают времена жизни полей экзистенциально и восстанавливают их при сопоставлении с образцом, а алгебраические обработчики эффектов передаются как значения-возможности.
Цель~--- дать машинно-проверенную метатеорию этого исчисления и точно установить, какие гарантии escape-анализа следуют из правил типизации.
Приводится формальная система: синтаксис, подтипизация времён жизни и типов, типизация с правилом минимума для конструкторов и устранением экзистенциальных времён жизни с учётом вариантности, а также малошаговая семантика с runtime-маркерами, делимитерами и реифицированными продолжениями.
Синтаксис, семантика и доказательства механизированы в Rocq/Coq на чистых индексах де Брёйна (около 29 тыс. строк; собранная разработка не содержит аксиом, незавершённых доказательств и гипотез).
...
Для исходных программ без каких-либо предположений о runtime-инвариантах доказаны три гарантии escape-анализа: каждое синтаксическое вхождение возможности имеет свой делимитер в области видимости, каждое выполненное событие на двух охраняемых каналах границы обработчика несёт значение нелокального типа, а значение, доставленное по нелокальному типу, не содержит возможностей и делимитеров ни на какой глубине.
Библиотека примеров содержит обработчики reader, state, exception и optionality с трассами редукции, а также машинно-проверенные теоремы об отклонении целых программ, нарушающих локальность.
Команда Print Assumptions подтверждает замкнутость всех ключевых теорем относительно глобального контекста, так что доверенная база сводится к ядру Rocq и его стандартной библиотеке.
Артефакт может служить проверенной спецификацией типизации для промежуточных языков компиляторов с capability-безопасными обработчиками эффектов; корректность размещения в памяти остаётся отдельным аргументом.}

\keywordsRus{системы типов, escape-анализ, экзистенциальные времена жизни, алгебраические обработчики эффектов, удержание capabilities, машинно-проверенная метатеория, Rocq/Coq.}
...
We study Core-Delta, a calculus in which lifetimes form a two-point lattice (free below local) with a minimum operation, compound data hide field lifetimes existentially and recover them on pattern matching, and algebraic effect handlers are passed as capability values.
The goal is a machine-checked metatheory of this calculus that states precisely which escape-analysis guarantees follow from the typing rules.
We give the formal system: syntax, lifetime and type subtyping, typing with a minimum rule for constructors and variance-aware elimination of existential lifetimes, and a small-step semantics with runtime markers, delimiters, and reified continuations.
Syntax, semantics, and proofs are mechanized in Rocq/Coq using pure de Bruijn indices (about 29 thousand lines; the built development contains no axioms, admitted proofs, or conjectures).
...
Three escape-analysis guarantees follow for source programs with no runtime-invariant assumptions: every capability occurrence has its delimiter in scope, every event on the two guarded boundary channels carries an escapably typed value, and values delivered at escapable types contain no capability or delimiter at any depth.
An example library exercises reader, state, exception, and optionality handlers with reduction traces, alongside machine-checked rejection theorems for complete leaking programs.
Print Assumptions reports all capstone theorems closed under the global context, so the trusted base is the Rocq kernel and its standard library.
The artifact can serve as a checked typing specification for compiler intermediate languages with capability-safe effect handlers; memory placement remains a separate argument.}
...
It originated as a compiler optimization, for example in stack allocation and the removal of intermediate objects~\cite{park1992escape,blanchet1999escape}.
In modern languages the same question is increasingly part of the type discipline: Rust ties it to lifetimes and borrowing~\cite{matsakis2014rust}, OCaml develops a related idea through a modal treatment of locality~\cite{lorenzen2024oxidizing}, recently extended to data-race freedom~\cite{georges2025data}, and Kotlin uses contracts for restricted forms of non-local control~\cite{akhin2021kotlin}.
Verifying such mechanisms has become a topic in its own right: RustBelt provides a semantic foundation for the safety of Rust~\cite{jung2018rustbelt}, complemented by the Stacked Borrows aliasing model~\cite{jung2020stacked}, while Prusti, Creusot, RustHornBelt, and RefinedRust lift properties of Rust programs to machine-checked specifications~\cite{astrauskas2022prusti,denis2022creusot,matsushita2022rusthornbelt,gaher2024refinedrust}.

A second reason to make escape analysis type-based comes from algebraic effect handlers.
Handlers replace a whole zoo of built-in control effects---exceptions, mutable state, generators, coroutines, asynchrony---with a single construct: a program \emph{performs} an operation and the nearest enclosing handler interprets it, with access to the captured continuation~\cite{plotkin2013handling}.
We carry this economy into the type system as well.
In a capability-based system an operation is invoked through a capability value; if that capability leaves the dynamic extent of its handler, the program would call a handler that no longer exists.
Effekt, capture tracking, and evidence-passing handlers address this by different means~\cite{brachthauser2020effects,brachthauser2022effects,boruch2023capturing,xie2021generalized,xie2022first}, usually through explicit capture sets or a substantial polymorphic infrastructure.
The $Core_\Delta$ calculus~\cite{stoyan2025existential} offers a coarser but compact alternative: lifetimes form a two-point lattice $\free \sqsubseteq \local$ with an explicit minimum, and compound data types hide the lifetimes of their fields existentially, recovering them on pattern matching.
Its design, subtyping, and a prototype type-checker are presented in that companion paper; the present work contributes the Rocq/Coq mechanization, the effect-handler extension with a marker-indexed semantics, and the machine-checked safety and escape guarantees.

This combination is convenient for the programmer but delicate for the metatheory.
Existential lifetimes interact with substitution, subtyping, and variance; handlers add continuation capture and runtime markers.
A pen-and-paper proof can easily miss a leak: a $\local$ value returned inside a closure's environment, or a capability captured by a resumption that outlives its handler.
The goal of this work is a machine-checked metatheory of $Core_\Delta$ extended with effect handlers, stating exactly which escape-analysis guarantees follow from the typing rules.
Section~\ref{sec:overview} introduces the analysis informally; Section~\ref{sec:calculus} formalizes it; Section~\ref{sec:mechanization} presents the mechanized metatheory; Section~\ref{sec:evaluation} evaluates the artifact.
...
First, we extend $Core_\Delta$ with multi-operation, capability-passing effect handlers and give a small-step semantics with runtime markers, delimiters, and reified continuations (Section~\ref{sec:calculus}).
Second, we mechanize the entire system in Rocq/Coq with pure de Bruijn indices: syntax, lifetime and type subtyping, typing---including the constructor \emph{minimum} rule and the variance-aware elimination of existential lifetimes at \texttt{match}---and operational semantics.
...
Fourth, we establish three escape-analysis guarantees as machine-checked statements about the \emph{dynamics}, all following for source programs with no runtime-invariant assumptions: every syntactic occurrence of a capability---including under $\lambda$s, in constructor arguments, and in stored operation bodies---has its marker's delimiter in scope (\texttt{source\_capability\_occurrence\_delimited}), every executed event on the boundary's two guarded channels---operation argument in, delimited-body answer out---carries an escapably typed value (\texttt{source\_boundary\_step\_noloc}), and a value delivered at any escapable type contains no capability or delimiter at any depth (\texttt{source\_noloc\_result\_no\_runtime\_forms}); together with type safety they are bundled into one record and one theorem, \texttt{source\_safety\_suite}.
...
None of this metatheory---progress, preservation, or any escape guarantee---appears in the companion paper, and mechanization materially changed the proof design: an early revision of \emph{this mechanization} assumed a marker-confinement axiom that proved false and was replaced by a proved invariant (Section~\ref{sec:mechanization}).

\section{Solution Overview}\label{sec:overview}
Before the formal development, we sketch the analysis informally, in the surface notation of the companion paper~\cite{stoyan2025existential}.
Types carry \emph{lifetime} labels: a type \lstinline[language=colang]|T| with lifetime \lstinline[language=colang]|lab| is written \lstinline[language=colang]{T'lab}; the label \lstinline[language=colang]|free| marks values that may escape, \lstinline[language=colang]|local| marks values confined to their defining scope, and a function type carries the lifetime of its \emph{closure}.
A compound value's lifetime is the minimum (written \lstinline[language=colang]|+|) of its components' lifetimes, and destructuring recovers component lifetimes from the container's; the companion paper develops this value discipline in detail, together with lifetime polymorphism and bounded quantification over tracked types.
This paper extends the same discipline to algebraic effect handlers.

Entering a $\keyword{handle}$ block mints a \emph{capability}---a first-class value granting the right to perform the effect's operations---and passes it to the handled code.
Besides aborting, a handler may run the suspended computation's \emph{continuation}, reified as a first-class \emph{resumption}; resuming once recovers reader- and state-like effects, resuming more than once expresses backtracking, and not resuming at all recovers exceptions.
A capability is meaningful only while its handler is on the stack: had it escaped the $\keyword{handle}$ block, a later $\keyword{perform}$ would address a handler that no longer exists.
Preventing that escape needs no new machinery: a capability is simply a value whose data type sits at lifetime $\local$, and a resumption is a function whose closure is $\local$.

Listing~\ref{lst:reader} shows the discipline on a reader effect.
The capability \texttt{c} has the $\local$ data type \lstinline[language=colang]{Reader<Int>'local}, so by the ordinary closure rule any function capturing it is itself $\local$ and cannot flow where a $\free$ value is expected.
The final function \texttt{leak} is rejected: its $\keyword{handle}$ body would answer at \lstinline[language=colang]{Reader<Int>'local}, and the handle rule requires the body's answer type to be non-local---the type system refuses to let the capability out.
...
Effects declare any number of operations; a handler supplies one clause per operation, and a $\keyword{perform}$ selects one by name.
Listing~\ref{lst:state} declares a two-operation state effect in state-passing style: each clause resumes and threads the state through a returned function.
Nothing about escape analysis changes---the one capability grants both operations, and the same $\local$ discipline confines it.

\begin{lstlisting}[language=colang,label={lst:state},caption={A two-operation state effect. One capability \texttt{st} grants both \texttt{get} and \texttt{put}; the handler threads the state through returned functions.}]
        effect State<s> { get: Unit -> s, put: s -> Unit }

        fun withState(body: (State<s>) -local-> r): (s) -local-> r =
            handle State<s> {
                get    => fun(s) => resume(s)(s)
                put(x) => fun(s) => resume(())(x)
            } in
                let y = body(st) in fun(s) => y
\end{lstlisting}

The guarantees we prove can be read informally.
First, a capability---every one of its syntactic occurrences, even under $\lambda$s and inside data---always sits under its own handler.
...
Third, a program typed at an escapable type only ever delivers values containing no capability or delimiter at any depth.
...
Classical region systems made lifetime an explicit object of typing long before modern ownership languages~\cite{tofte1997region,fluet2006monadic}.
Their aim is close to ours: to replace a global analysis with local conditions at function boundaries.
The difference is that $Core_\Delta$ does not parameterize every user type by a lifetime.
A constructor's result restriction must be at least as restrictive as the join of its fields' restrictions, and pattern matching introduces fresh existential variables bounded by the scrutinee's lifetime.
This reduces annotation but complicates the proof of type preservation under reduction.

Effect handlers have been studied denotationally and operationally~\cite{plotkin2013handling,zyuzin2021contextual,van2022handling,van2024framework,tang2025modal}, integrated into OCaml and WebAssembly~\cite{sivaramakrishnan2021retrofitting,phipps2023continuing}, and compiled via evidence passing~\cite{xie2021generalized}.
...
Closest are the capability-based systems---Effekt's scope- and type-based reasoning~\cite{brachthauser2020effects,brachthauser2022effects} and capture tracking~\cite{boruch2023capturing}; a recent modal framework compares row- and capability-based systems in one setting~\cite{tang2026modal}.
$Core_\Delta$ records coarser information---a value is either free to escape or locally confined---and, unlike systems that extend the type language with rows or capture sets, adds no effect-specific type structure: the machinery that bounds value escape also confines capabilities.

Capability confinement itself has many routes: lexically scoped handler instances~\cite{biernacki2019binders} and scoped operations~\cite{wu2014effect}; provenance through the typing context in Scala~\cite{odersky2021safer,odersky2022scoped} and mainstream context-passing~\cite{KEEP-context-parameters}; second-class values~\cite{osvald2016gentrification}; effect rows~\cite{leijen2017type}; evidence- and tunnel-based designs~\cite{zhang2016accepting,zhang2020handling}; and mechanized capture calculi with explicit boxes~\cite{xu2023formalizing} or, recently, without them~\cite{xu2025s}---a broader survey situates these lines~\cite{stoyan2024modern}.
$Core_\Delta$ folds confinement into the two-point value-lifetime discipline it already uses for ordinary data.

Mechanizing type-system metatheory builds on POPLMark and the engineering of formal metatheory~\cite{aydemir2005poplmark,aydemir2008engineering}, with syntax and substitution automation available~\cite{sewell2010ott,stark2019autosubst2}; handler metatheory exists in separation-logic form~\cite{devilhena2021separation}.
...
The model must therefore (i) express lifetimes in types without overloading the user with parameters; (ii) confine a runtime capability to its handler; (iii) admit first-class continuations while keeping them off every escapable channel; and (iv) have a checkable metatheory suitable as a compiler specification.
...
Lifetimes are the two-point lattice with a minimum: $\Delta_1+\Delta_2$ denotes the lifetime \emph{minimum}---the least upper bound in $\free \sqsubseteq \local$, since the more restrictive (shorter) lifetime sits \emph{higher} in the order---$\local$ as soon as either operand is $\local$.
...
Core functions are unary; multi-argument functions and surface effect declarations are sugar; type and lifetime abstractions are prenex---in the artifact every $\Lambda$-chain ends in a $\lambda$.
...
The reified resumption $\reic_m^{\,\tau,\sigma}(x.\,t)$ is \emph{derived notation}, not a constructor: it abbreviates the delimiter-re-installing abstraction $\lambda(x{:}A).\,\hdlr_m^{\,\tau,\sigma}\,t$ (Figure~\ref{fig:handlers}, caption), and is therefore a value because $\lambda$-abstractions are.
...
A capability value has a $\local$ data type, and a resumption is a function with closure lifetime $\local$ (Figure~\ref{fig:handlers}, top): both are therefore barred from every escapable channel by the function discipline above.
...
The runtime forms mirror this: a capability records $T_R$ and types its stored operation clauses the same way (\rname{T-Cap}), and a handler delimiter is transparent to typing, requiring only that the body answer type $T_B$ be non-local and a subtype of the public answer $T_R$ (\rname{T-HandlerM}).
The \rname{T-Perform} rule selects an operation $op$ of the capability's effect and instantiates its signature once more with the perform-site type arguments $\overline{S}$, giving argument type $\sigma^{op}_{\overline{S}}$ and result type $\rho^{op}_{\overline{S}}$; it requires the argument type and each $S_i$ to be non-local---$\noloc_\Gamma(\tau)$ abbreviates $\lsub{\Gamma}{\ltof_\Gamma(\tau)}{\free}$---so a value crossing the operation boundary cannot smuggle a local capability out of scope.
...
The operational rules (Figure~\ref{fig:handlers}, bottom) are layered: head reductions $t \hstep t'$ have no congruence, and $t \step t'$ closes them under well-formed call-by-value evaluation contexts $\mathcal{E}$ via $\rname{S-Step}$.
...
A $\keyword{perform}$ of operation $op$ on a capability with marker $m$, sitting inside a context $P$ that is pure (delimiter-free) for $m$ and a well-formed call-by-value evaluation context, under the matching $\hdlr_m$, captures $P$ as a resumption $\reic_m$ that re-installs the delimiter, and runs \emph{that operation's} stored clause on the argument and the resumption ($\rname{H-Perform}$).
...
The invariant $\mathsf{well\_scoped}\;\overline{m}\;t$ records that every runtime capability with marker $m$ lies under its $\hdlr_m$ or within the active set $\overline{m}$, together with a provenance condition on stored operation bodies (Section~\ref{sec:mechanization}); source programs satisfy $\mathsf{well\_scoped}\;[\,]\;t$.
...
$\infer[\rname{T-Handle}]{\tj{\Gamma}{\keyword{handle}\;E\,[\overline{\tau}]\,\{\overline{op\Rightarrow t_{op}}\}\;\keyword{in}\;b}{T_R}}
  {\begin{array}{c}
   \overline{\tj{\Gamma,x{:}\sigma^{op}_\beta,\,k{:}\rho^{op}_\beta\arr{\local}T_R}{t_{op}}{T_R}} \\[1pt]
   \tj{\Gamma,c{:}E\,\local\,\overline{\tau}}{b}{T_B} \quad
   \noloc_\Gamma(T_B) \quad
   \tsub{\Gamma}{T_B}{T_R}
   \end{array}}$
\\[1.6ex]
$\infer[\rname{T-Cap}]{\tj{\Gamma}{\cap_E^{m,T_R}[\overline{\tau}]\{\overline{op\Rightarrow t_{op}}\}}{E\,\local\,\overline{\tau}}}{\overline{\tj{\Gamma,x{:}\sigma^{op}_\beta,\,k{:}\rho^{op}_\beta\arr{\local}T_R}{t_{op}}{T_R}}}$
\quad
...
\quad (\rname{H-Perform}; $P$ pure for $m$ and well-formed)
\\[1.2ex]
$(\reic_m^{\,T_B,T_R}(x.b))\,v \hstep \hdlr_m^{\,T_B,T_R}\,[x{\mapsto}v]b$
\quad (\rname{H-Resume})
\qquad
$\infer[\rname{S-Step}]{\mathcal{E}[q]\step \mathcal{E}[q']}{q\hstep q'}$
\qquad
...
\caption{Handler typing (top) and operational semantics (bottom).
Overlined premises and clauses range over the effect's declared operations, which the clause list must cover exactly; $op\in E$ abbreviates looking up $op$'s signature in $E$'s declaration.
$\sigma^{op}_\beta,\rho^{op}_\beta$ (resp.\ $\sigma^{op}_{\overline{S}},\rho^{op}_{\overline{S}}$) are operation $op$'s argument/result types instantiated at the effect's type arguments $\overline{\tau}$ (resp.\ additionally at the perform-site arguments $\overline{S}$); effect lookup, well-formedness, and length premises are elided.
...
In \rname{H-Perform}, $t_{op}$ is the stored clause that the index $op$ selects, and $P$ must be both pure for $m$ and a well-formed evaluation context ($\mathsf{ectx\_wf}$ in the artifact)---purity picks the delimiter, well-formedness the unique call-by-value capture point; $\overline{\beta}$ are the selected operation's type parameters, instantiated at the perform-site arguments $\overline{S}$ before the term substitution; in the mechanization the captured context $P$ is also shifted across the resumption's new $\lambda$-binder.
...
The development is in Rocq/Coq, using pure de Bruijn indices and ordinary recursive shift/substitution functions, and is published at \url{https://github.com/penguin-boxx/effect-proofs}.
The verified core is the modules of Table~\ref{tab:modules}, which build in dependency order---\texttt{core/} fixes the calculus, \texttt{meta/} proves the substitution metatheory, and \texttt{safety/} derives the guarantees; example files are separated and are not part of the trusted base.
...
\texttt{meta/} & shift and substitution commutation laws, weakening, narrowing, substitution-preserves-typing lemmas, closedness bookkeeping \\
...
The headline result is stated on \emph{executed events}: \texttt{source\_boundary\_step\_noloc} shows that every boundary transition on either channel carries a value typed at \emph{that channel's declared type}, which the typing rules force to be escapable (conclusion \texttt{boundary\_channel\_typed}); a state-shaped form (\texttt{source\_handler\_boundary\_noloc}) and a data-type specialization (\texttt{source\_boundary\_value\_non\_local}: a constructor with non-local top-level annotation) accompany it.
...
A subtyping-level companion, \texttt{local\_data\_not\_escapes}, shows the coercion $\tsub{\Gamma}{K\,\local\,\overline{\tau}}{K\,\free\,\overline{\tau}}$ underivable in any runtime environment: local data cannot even be retyped as free, let alone delivered.
The type information therefore forbids the observable leakage of local values.

\emph{Guarantee suite.} The principal source-facing guarantees---type safety, invariant preservation, both confinement forms, guarded-channel safety, resumption locality, and depth non-escape---are bundled as one record and one theorem: \texttt{source\_safety\_suite} (\texttt{Guarantees.v}), a single statement with a single \texttt{Print Assumptions} target.
...
The development contains no \texttt{Axiom}, no \texttt{Admitted}, and no \texttt{Conjecture}: \texttt{Print Assumptions} on every capstone theorem of this section---including \texttt{type\_soundness}, the source-program corollaries, and \texttt{source\_safety\_suite}---reports \emph{closed under the global context}, so the trusted base is the Rocq kernel and its standard library.

The well-scopedness invariant deserves a proof-engineering comment.
An early revision assumed a marker-confinement \emph{axiom} that turned out to be false as stated; it was replaced by the explicit structural invariant $\mathsf{well\_scoped}$, first carried as a hypothesis of the capstone theorem and finally proved preserved by reduction.
The step that made the preservation proof go through was reformulating the invariant \emph{without} a marker-freshness requirement: the final predicate records \emph{provenance}---each capability's stored operation body must be well-scoped at the scope outside that capability's own delimiter, exactly what the $\keyword{perform}$ contraction needs---and is monotone under scope extension, so installing a new delimiter around a term or moving it under binders by substitution cannot invalidate it.
The operational semantics still mints globally fresh markers in \rname{S-Handle}; what turned out to be unnecessary is carrying freshness in the \emph{preserved invariant}: $\beta$-substitution duplicates subterms and so destroys uniqueness, but the semantics always addresses the \emph{innermost} occurrence of a marker, so shadowing is harmless.
...
The evaluation has three parts---reproducibility, positive examples, and negative witnesses---followed by a comparison with related systems and a note on practical use.
...
The development targets Rocq~9.1; exactly the files listed in \texttt{src/\_CoqProject} constitute the verified development, and the example files are outside the trusted base.
...
The repository ships under the MIT license with a generated theorem index and build statistics (\texttt{STATS.md}), and the artifact guide scripts the tag-release-archive procedure (GitHub release plus Zenodo DOI) for the reviewed commit.

\emph{Positive examples.} \texttt{Examples.v}/\texttt{ExamplesProofs.v} mechanize the programs of Listings~\ref{lst:reader} and~\ref{lst:state} together with exception and optionality handlers, each with a machine-checked typing proof and a reduction trace.
On the reader program the boundary guarantee is instantiated concretely (\texttt{reader\_example\_boundary\_noloc} specializes \texttt{source\_handler\_boundary\_noloc} to its execution), and the rejected \texttt{leak} function fails exactly the $\noloc_\Gamma(T_B)$ premise of \rname{T-Handle}.
On the two-operation \texttt{StateM} run of Listing~\ref{lst:state}, the capstone guarantees---soundness, confinement, and a boundary event---are witnessed directly (\texttt{ExamplesSafety.v}).
A delegating handler, whose operation clause performs on an outer capability, carries its own safety witnesses, alongside bounded sums and long many-$\keyword{perform}$ runs.
The library also exercises the resumption discipline beyond one-shot use: a multi-shot handler whose operation body resumes twice (\texttt{multishot\_example}, with a machine-checked reduction trace evaluating to $3$) and a forwarding example that nests handlers.
...
The offending program stores a reader capability in an \texttt{Option} and hands it out at the escapable interface type $\mathtt{Option}\,\free\,(\mathit{Reader}\,\local\,\mathsf{Unit})$.
...
Directly returning a capability at the escapable interface $\mathtt{Option}\,\free\,(\mathit{Reader}\,\local\,\mathsf{Unit})$ is not typable at that interface (\texttt{leak\_reader\_rejected}), while the same value is typable \emph{inside} the handler at a $\local$ type (\texttt{some\_capability\_typable\_inside}): the analysis rejects the escape, not the value.
...
Table~\ref{tab:comparison} situates the work.
Unlike Rust and OCaml \texttt{local}, the model has first-class capability values and resumptions; unlike Effekt and capture tracking, it uses an explicit lifetime minimum instead of capture sets.
The ``Mech.'' column only records \emph{whether} a mechanization exists and flattens very different depths---RustBelt is a far deeper semantic model than our syntactic type-safety result---so the table is a coarse map, not a ranking.
In particular, Effekt couples its handlers with capture-set-style polymorphism while $Core_\Delta$ keeps the type language bare, so a ``yes'' in the Eff.\ column is a coarse match rather than a claim of equivalence.
The Min column, in turn, proxies the deeper distinction: this work adds no effect-specific type \emph{former}---no rows, capture sets, or boxes.
Every row refers to the cited formal system rather than the full language it comes from.

\begin{table}[h]
\centering
\caption{Comparison with related systems (Mech.\ = mechanized; EA = escape analysis; Eff.\ = effect handlers; Min = explicit lifetime minimum; ``indirect'' = the minimum arises through outlives constraints rather than an explicit operator).}
...
Confinement is per-marker, not per-operation: all operations of one handler share its delimiter and its public answer type $T_R$.
...
We presented a machine-checked metatheory of $Core_\Delta$ extended with multi-operation algebraic effect handlers, mechanizing the full system---syntax, lifetime and type subtyping, typing with a constructor minimum rule and variance-aware elimination of existential lifetimes, and a marker-indexed small-step semantics---and proving progress, preservation, and type safety with no runtime-invariant assumptions for source programs, together with occurrence-level capability confinement, guarded-channel safety, and depth non-escape of local values.
...
The artifact is useful as a checked typing specification for compiler intermediate languages with first-class capability values and capability-safe effect handlers; widening the lifetime lattice to multi-level regions, connecting to a memory model, and extracting a certified checker are the natural next steps.
