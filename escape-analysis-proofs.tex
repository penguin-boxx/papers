...
В работе рассматривается исчисление Core-Delta, в котором времена жизни образуют двухточечную решётку с операцией минимума (формально — наименьшая верхняя грань решётки), составные данные скрывают времена жизни полей экзистенциально и восстанавливают их при сопоставлении с образцом, а алгебраические обработчики эффектов передаются как значения-возможности.
...
Приводится формальная система: синтаксис, подтипизация времён жизни и типов, типизация с правилом минимума для конструкторов и устранением экзистенциальных времён жизни с учётом вариантности, а также семантика редукций с маркерами времени выполнения, делимитерами и реифицированными продолжениями.
Синтаксис, семантика и доказательства механизированы в Rocq/Coq на чистых индексах де Брёйна (около 30 тыс. строк, без аксиом, незавершённых доказательств и гипотез).
...
Syntax, semantics, and proofs are mechanized in Rocq/Coq using pure de Bruijn indices (about 30 thousand lines, with no axioms, admitted proofs, or conjectures).
...
Its design, type system, and a prototype type-checker are presented there.
...
Handlers are the \emph{sharpest instance} of scope-tied meaning: in a capability-passing discipline an operation is invoked through a capability value, valid only while its handler is on the stack.
...
Within these bounds the result is a faithful, checkable specification with an executable reduction semantics and escape-side-condition deciders.
...
Fifth, we provide a mechanized example library with executable reduction traces and machine-checked \emph{negative} witnesses that reject locality-violating programs (Section~\ref{sec:evaluation}).
...
We extend the same discipline to algebraic effect handlers.
...
User-defined types need not expose lifetime parameters; only functions abstract over lifetimes in the surface language, and a constructor instead absorbs the parts' lifetimes into the annotation of the whole:
\begin{lstlisting}[language=colang]
    fun makeRepo<lf,lc>(f: File'lf, c: Connection'lc): Repository'lf+lc =
        Repository(f, c)
\end{lstlisting}
...
First, a capability---every one of its syntactic occurrences, even under $\lambda$s and inside data---always sits under a delimiter of its handler.
Second, the two guarded data channels crossing the handler boundary (the operation argument going in, the delimited body's answer coming out) only ever carry values of escapable types.
...
Effect handlers have been studied denotationally and operationally~\cite{plotkin2013handling,zyuzin2021contextual,van2022handling,van2024framework,tang2025modal}, integrated into OCaml, WebAssembly, and Unison~\cite{sivaramakrishnan2021retrofitting,phipps2023continuing,unison-abilities}, and compiled via evidence passing~\cite{xie2021generalized}.
...
Core functions are unary; multi-argument functions and surface effect declarations are sugar, and the surface angle-bracket instantiation of Section~\ref{sec:overview} corresponds to the formal type and lifetime applications $t\,[\tau]$, $t\,[\Delta]$; type and lifetime abstractions are restricted so that every $\Lambda$-chain ends in a $\lambda$.
...
The reduction rules (Figure~\ref{fig:handlers-sem}) are layered: head reductions $t \hstep t'$ have no congruence, and $t \step t'$ closes them under well-formed call-by-value evaluation contexts $\mathcal{E}$ via $\rname{S-Step}$; handler allocation ($\rname{S-Handle}$) is the second step-level rule, not a head reduction.
...
The named context order is schematic; in the artifact the operation argument is term index $0$ and the resumption index $1$.}
...
\qquad
$\infer[\rname{S-Handle}]{\mathcal{E}[\keyword{handle}\;E\,[\overline{\tau}]\,\{\overline{op{\Rightarrow}t_{op}}\}\;\keyword{in}\;b]\step \mathcal{E}[\hdlr_m^{\,T_B,T_R}\,([c{\mapsto}\cap_E^{m,T_R}]\,b)]}{\mathsf{ectx\_wf}(\mathcal{E}) & m\ \text{globally fresh}}$
...
The step function decides one step and its driver is fuel-bounded; termination of the calculus is neither claimed nor proved.
...
One bookkeeping caveat: $\mathsf{boundary\_step}$ is a labelled subrelation of reduction---every labelled event is a genuine step and satisfies its channel guarantee, but the completeness of this five-flow accounting over all reduction steps is established by inspection of the semantics, not as a Rocq theorem.
...
The development is about $3\times10^{4}$ lines with about $1.1\times10^{3}$ \texttt{Qed} obligations; the substitution metatheory (\texttt{meta/}) alone accounts for roughly $47\%$ of the lines, the price of pure de Bruijn indices with hand-rolled shift and substitution functions.
...
The development targets Rocq~9.1; exactly the files listed in \texttt{src/\_CoqProject} constitute the verified development; all example files are compiled, the safety and complete-rejection files are additionally designated capstone gate files, and no metatheory result depends on them.
A separate target, \texttt{make check-assumptions}, re-runs the \texttt{Print Assumptions} gate over a capstone list \emph{derived} from the designated capstone files (including the example safety and rejection files), and \texttt{make verify} adds a documentation-freshness gate; both run in continuous integration, and the maintenance tooling additionally requires Python~3, while the verified development itself needs only Rocq.
...
Rejection at typing time is the static complement of the dynamic guarantees of Section~\ref{sec:mechanization}---the leaking witnesses are untypable at their advertised interfaces, and well-typed source programs provably do not leak.

Table~\ref{tab:comparison} situates the work; every row refers to the cited formal system rather than the full language it comes from, and the Mech.\ column records machine-checked metatheory for that cited model while flattening very different depths---RustBelt is a far deeper semantic model than our syntactic result.
Unlike Rust and OCaml \texttt{local}, the model has first-class capability values and resumptions; unlike Effekt and capture tracking, it hides field lifetimes existentially and adds no extra tracking structure to the type language.
In theorem strength, the cited mechanized capability and capture calculi prove type soundness for their core systems; the guarantees here are \emph{dynamic} confinement statements---occurrence-level delimitation and per-event channel typing---obtained without extending the type language.

\begin{table}[h]
\centering
\caption{Comparison with related systems (Mech.\ = machine-checked metatheory of the cited core model; EA = type-level escape control for values; Caps = first-class handler capabilities and resumptions, ``boxed'' = first-class only via explicit boxing; Exist.\ = existential field lifetimes; Extra = additional effect- or capture-tracking type structure beyond value lifetimes, such as rows, capture sets, or boxes).}
\label{tab:comparison}
\footnotesize
\begin{tabular}{@{}p{0.30\linewidth}ccccc@{}}
\toprule
\textbf{System} & \textbf{Mech.} & \textbf{EA} & \textbf{Caps} & \textbf{Exist.} & \textbf{Extra} \\
\midrule
Rust/RustBelt~\cite{matsakis2014rust,jung2018rustbelt} & yes & yes & no & no & no \\
OCaml \texttt{local}~\cite{lorenzen2024oxidizing} & no & yes & no & no & no \\
...
Runtime confinement is per-marker, not per-operation: all operations of one handler share its marker and its public answer type $T_R$.
The static side is coarser still: lifetime classification is global---all confined values share the single $\local$ point---so a local value is blocked by every guarded handler boundary, not only by the delimiter that made it local.
There is also no type-and-effect accounting: function types do not record which operations a computation may perform, and \texttt{source\_effect\_safety} asserts delimiter availability and confinement, not row-based effect tracking.
...
The syntax, the substitution tier, and the variance-aware elimination already operate over arbitrary lifetime expressions (\rname{T-Match} bounds its fresh variables by an arbitrary $\Delta$) and should be reusable; widening the constants to a genuine region hierarchy would still require redesigning lifetime subtyping, the escape predicate and its decider, capability typing, and the non-escape proofs.
...
A mechanized example library with executable reduction traces and machine-checked negative witnesses shows the analysis on reader/state/exception/optionality handlers and on programs it must reject.
