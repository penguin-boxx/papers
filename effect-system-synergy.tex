...
%    frameround=tttt,                      % rounded corners on frame
    ...
    morekeywords={*,...},                % add additional keywords here
    gobble=4,      % skip 4 characters of indentation in code lines
}
\lstdefinelanguage{colang}{
    keywords=[1]{let, effect, perform, data, context, fun, if, else, throw, try, catch, local, free, op},
    sensitive=true,
    comment=[l]{//},
    morecomment=[s]{/*}{*/},
    morestring=[b]",
...
\begin{lstlisting}[language=colang]
    ...
    context(io: IO) // io: IO'l1
    fun printAll(xs: LazyList<Int>) {
        let ys: LazyList<Int>'l1 = lazyMap(xs, (x) => io.print(x))
...
\begin{lstlisting}[language=colang]
    fun first(x: Int'l1, y: Int'l2): Int'l1+l2 {
        let intPair: IntPair'l1+l2 = IntPair(x, y)
        return intPair.fst
    }
\end{lstlisting}
...
Various techniques can be employed to reduce boilerplate associated with lifetime polymorphism and the specification of bounds. % todo dependent sugar
...
We will discuss this technique, along with alternative methods, in Section~\ref{subsec:lifetime-poly-enhancement}.
...
The following computation sums the results of two invocations of \texttt{ask}; with implementation returning the constant $21$, the overall result of the computation will be $42$: % todo this is a `context let`
\[
    \begin{array}{l}
...
\begin{lstlisting}[language=colang]
    fun compose<a <: Any', b <: Any', c <: Any'>(f: (b)' -> c, g: (a)' -> b): (a)' -> c
\end{lstlisting}
...
\begin{lstlisting}[language=colang]
    fun compose<a, b, c>(f: (b) -> c, g: (a) -> b): (a)'f+g -> c
\end{lstlisting}
