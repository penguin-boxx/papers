...
\begin{itemize}
    ...
    \item The typing context is an ordered list.
    $\in_{first}$ relation is only satisfied when the latest entry in a typing context has given name and binding type ($:$ or $:_c$). % todo clarify
\end{itemize}
...
\begin{figure}
    ...
    \begin{gather*}
        ...
        \text{\hspace{2em}}
        \infer[CVar]{\Gamma\vdash x : \tau}{x :_c \tau \in_{first} \Gamma} \\
        \infer[Lam]{
...
\subsubsection{Typing rules for $F_{im}$ (fig.\ \ref{fig:fim-typing})}

Typing rule for an omitting application needs to check if all implicits can be satisfied
...
\begin{figure}
    ...
    \begin{gather*}
        ...
        \infer[CtxApp]{
            ...
            \Gamma\vdash t : \ctx{e} \overline{\tau} \to \sigma &
            e\subset_c\Gamma &
            \overline{\Gamma\vdash u : \tau}
        }
%        \mathframebox{\sigma <: \tau} \\
%        \infer[Sub]{\ctx{e} \tau\to\sigma <: \ctx{e'}\tau\to\sigma}{e' <: e}
    \end{gather*}
    \caption{Additional typing rules for $F_{im}$.}
    \label{fig:fim-typing}
\end{figure}
