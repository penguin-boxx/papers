...
%Values are variables, type abstraction, data constructor application and lambda abstraction.
We use uncurried abstraction and application for simplicity to make implicit parameters inference local in a sense that typing information available to an application rule is enough (no deferred constraint solving is required). % todo link Haskell
Functions have two blocks of arguments, first of which is for future implicit inference.
We use lexical handlers with explicit user-defined names.
The \texttt{handler} construct is only needed as a reduction step for operational semantics.
Also for brevity, we do not include \texttt{return} block in handlers, because it does not add any expressiveness.
...
\begin{figure}
    ...
    % todo beautify
    % todo syntax of execution contexts
...
\begin{figure}
    \[
        \begin{array}{llll}
            ...
            \\
            \text{(let)} &let ~x = t ~in~u &\longrightarrow &[x \to t]\ap u
            \\
            \text{(match)} &match ~K \ap \overline{t}~ with ~ \{ \overline{K_i \ap \overline{x} \to u_i}\} &\longrightarrow &[\overline{x\to t}]\ap u_k
            \\
            &&&\text{where } K = K_k
            \\
            \text{(handle)} &handle ~ x : T \ap \overline{\tau} ~with~h~in~t &\longrightarrow & handler_m ~h ~ in~ [x \to K \ap m \ap \overline{t_i}] \ap u
            \\
            &&&\text{where } m \text{ fresh}, \{\overline{f_i = t_i}\} \subset h
            \\
...
\subsection{Algorithmic implicit parameters inference}
