...
It allows to type the abstraction over execution contexts. % todo rephrase
...
% TODO one typing context vs two
...
Actually, the decision is about the default, since one behaviour can be changed to another by $\eta$-expansion. % todo explain better, examples
...
\begin{figure}
    ...
    \begin{gather*}
        ...
        \text{\hspace{1em}}
        \infer[AppArgCtx]{
            \Gamma \vdash t \Leftarrow ctx(\overline{\tau}) ~\overline{\sigma}\to\eta\step \lambda \overline{x}~\overline{y}\ldotp t_c\ap \overline{x'}\ap\overline{y}
        }{
            \Gamma \vdash t : ctx(\overline{\sigma}) ~\overline{\sigma}\to\eta \step t_c
            &
            \overline{x :_c \tau} \vdash_{\overline{\sigma}} \overline{x'}
            &
