...
We use uncurried abstraction and application for simplicity to make implicit parameters inference local in a sense that typing information available to an application rule is enough.
...
\begin{figure}
    \[
        \begin{array}{llll}
            ...
            \\
            \text{(return)} &handler_m ~h ~ in ~v &\longrightarrow &t \ap v
            \\
            &&&\text{where } \{return = t\} \subset h
            \\
            \text{(perform)} &handler_m~h~in~E[perform \ap f_j\ap (K \ap m \ap \overline{t_i})\ap\overline{\tau}\ap\overline{v}] &\longrightarrow &t_j \ap \overline{\tau}\ap \overline{v}\ap k
            \\
