...
For example, the lifetime elision from the Rust language~\cite{matsakis2014rust} uses heuristics to automatically place lifetime variables in a reasonable in most situations way (we use \texttt{scoped} keyword without a particular lifetime to trigger an elision mechanism).
...
\begin{figure}
    ...
    \[
        \begin{array}{lrlll}
            ...
            \\
            \text{Terms} &t, u &\Coloneqq \vor v \vor t\ap\overline{\tau} \vor t \ap \overline{t} \ap \overline{t} \\
        ...
        \begin{array}{lrl}
            ...
            \text{Evaluation context} &E &\Coloneqq \vor \square \vor E \ap \overline{\tau} \vor E \ap \overline{v}\ap \overline{t} \\
...
\begin{itemize}
    \item We use uncurried abstraction and application for simplicity to make implicit parameters inference local in a sense that typing information available to an application rule is enough (no deferred constraint solving is required). % todo link something
    \item We couple the first block with implicit parameters and the second block with ordinary ones to make sure that functions are never partially applied to implicit arguments to avoid accidental capturing. % todo example, investigate Kotlin
    ...
    \item % todo lexical handlers stuff
    \item We need the \texttt{handler} construct as a reduction step for operational semantics.
    \item For brevity, we do not include \texttt{return} block in handlers, because it does not add any expressiveness.
    \item Implicit parameters are a part of a function type to support the abstraction over execution contexts:
    \begin{minted}{swift}
        func withSomeException(f: context(Handler<SomeException>) () -> r): r =
            try { f() } catch (e: SomeException) { ... }
    \end{minted}
    \item Typing contexts distinguish between regular bindings and implicit bindings.
    Implicit ones are assigned with monotypes, implicits resolution based on schemas appears to be more complicated.
\end{itemize}
