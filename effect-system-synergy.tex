...
An opportunity to close over capabilities support a technique called \textit{contextual polymorphism}, which allows higher-order functions to be transparent regarding effects without additional polymorphic type variables ranging over effects. % todo citations (contextual polymorphism, OOP and higher-order)
...
\begin{minted}{swift}
    ...
    context(Handler<SomeException>)
    func caller(xs: List<Int>) {
        map(xs, (x) => throw SomeException()) // capability capturing is safe here
    }
\end{minted}
...
Let us five an informal brief introduction of the type-based escape analysis technique that we feature in this paper.
...
Lifetime labels range through \texttt{free}, \texttt{local} and polymorphic lifetime variables.
...
If a function returns a value that captures some of the arguments, it can use polymorphic lifetime variables to explicitly specify its dataflow.
...
Such a function still supports contextual polymorphism since it can leak \texttt{f} only with a result and a call site is aware that this result should not escape as well:
\begin{minted}{swift}
    func lazyMap(xs: LazyList<a>, f: scoped(l) (a) -> b): scoped(l) LazyList<b>

    context(io: IO) // io: scoped(local) IO
    func printPlusOne(xs: LazyList<Int>) {
...
For simplicity, we get rid of lifetime polymorphism for compound datatypes by computing their lifetimes as an intersection of component's lifetimes.
Lifetimes of components themselves on destructuring are approximated with a lifetime of a compound datatype.
\begin{minted}{swift}
    func compound(x: scoped(l1) Int, y: scoped(l2) Int): scoped(l1&l2) Int {
...
A term depends on a context with its free variables: in different contexts it obtains different semantics.
...
We summarize how an effect system manages with free variables in the table~\ref{fig:free-vars}.

\begin{figure}
    \centering
    \begin{tabular}{|l|l|l|}
        \hline
        Scoping of free variables & dynamic & static \\
        \hline
