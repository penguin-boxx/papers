...
\begin{itemize}
    \item The functional type $\ctx{e}\overline{\tau}\to\sigma$ ensures that implicit parameters should always be followed with ordinary ones to avoid computations being forced by inference of implicits (the overline denotes a tuple of argument types, with the empty tuple interpreted as the unit type).
    \item Following the functional type, on the term level we split function arguments into two groups: \emph{contextual arguments}, which will be inferred automatically in future extensions, and ordinary arguments.
    ...
    \begin{gather*}
        ...
        \infer[Handle]{
            ...
            \begin{array}{ll}
                ...
                \Sigma(T) = \forall\overline{\alpha}\ldotp sig, \hspace{0.5em} op_i : \forall\overline{\beta}_i\ldotp \overline{\sigma}_i\to\sigma'_i \in sig
                \\
...
\begin{figure}
    \begin{gather*}
        ...
        \\
        \infer[Lam]{
            \Gamma\vdash \lambda \overline{x : \tau} ~ \overline{y : \sigma} \ldotp t : ctx(\overline{\tau}) \left(\&{\overline{lt_+(s)}}\right)~ \overline{\sigma} \to \eta
        }{
            \overline{x :_c \tau}, \overline{y : \sigma}, \Gamma\vdash t : \eta &
            \Gamma\vdash_{fv(t)}\overline{s} &
            local\not\in lt_+(\eta)
        } \\
        \infer[App]{
    ...
    % todo что с полярностью на конструкторах
\end{figure}
...
This $regional$ lifetime differs from $local$ in two key aspects: it can bind generics of effectful operations, and it can appear within the parameter types of operations.
