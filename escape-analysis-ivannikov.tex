...
%        National Research University Higher School of Economics, Saint Petersburg, Russian Federation
    ...
    To achieve high precision and efficiency, particularly in interprocedural contexts, this analysis is often implemented on the type level.
...
%    Presence of these labels in type signatures makes the analysis local
    ...
    Despite it's usefulness and interprocedural abilities, languages often avoid using type-based escape analysis, preferring classical approaches based on monotone framework~\cite{kam1977monotone, blanchet1999escape}.
    ...
    Also, our approach incorporates union types, which make type signatures transparently describe a function's dataflow.
    ...
    To differentiate between tracked and non-tracked types, each type \lstinline[language=colang]|T| is annotated with a special label \lstinline[language=colang]|lab|, referred to as a lifetime: \lstinline[language=colang]{T'lab}; or \lstinline[language=colang]{(A)'lab -> B} for function types.
