...
%        National Research University Higher School of Economics, Saint Petersburg, Russian Federation
    ...
    In this paper, we introduce a novel type-based escape analysis that simplifies lifetime tracking for data types through the use of existential lifetimes.
    Additionally, our escape-analysis incorporates a minimum operation on lifetimes, which enables user-provided type annotations to more accurately capture a function’s data flow, thereby making them simpler and more intuitive to write.
...
%    Presence of these labels in type signatures makes the analysis local
    ...
    This technique is widely used by optimizing compilers, for example, to facilitate stack allocation of objects that would otherwise be allocated on the heap, thereby reducing garbage collection overhead and enhancing overall program performance~\cite{blanchet1999escape}.
    ...
    Specifically, we introduce existential lifetimes to automate the handling of lifetimes within data types.
    Additionally, our approach incorporates a minimum operation on lifetimes, enabling type signatures to transparently reflect a function's data flow.
    ...
    This approach effectively encodes the function’s data flow within the type signature.
    ...
    For the sake of simplicity, lifetime polymorphism is not supported for compound data types.
    ...
    The type-level syntax of $Core_\Delta$ is given in Figure\ \ref{fig:core-types}.
    We use the $+$ operator on lifetimes in prefix position when taking the minimum of a sequence of lifetimes (e.g.\ $+\overline{\Delta}$)\footnote{Effectively, the empty list of lifetimes in the minimum denotes the \lstinline[language=colang]{free} lifetime. We discuss normalized presentation later in Section\ \ref{sec:implementation}, but in the paper, we use the direct definition for clarity.} and in notation for specific lifetimes (e.g.\ $\Delta_1 + \Delta_2$).
    ...
    \begin{figure}
        \[
            ...
            \end{array}
        \]
        \caption{Type syntax of $Core_\Delta$.}
    ...
    Consequently, the minimum of a set of lifetimes corresponds to the least upper bound within this lattice, making it analogous to a union type operation\footnote{Note that so-called ``minimum'' of lifetimes correspond to ``maximum'' in terms of lattice theory.}.
    ...
    The last rule excludes the influence of quantified parameters from the result by changing the bounds for $\overline{\alpha}$ and removing all occurrences of $\overline{l}$.

    The $SubAny$ rule establishes that the type $Any\ap \Delta$ is a supertype of $\tau$ if and only if all constituent components of $\tau$ outlive $\Delta$.
...
%    \end{itemize}
    ...
    \begin{figure}
        \begin{gather*}
            ...
            \end{array}
        \end{gather*}
        \caption{Typing basic constructs of $Core_\Delta$.}
    ...
    \begin{figure}
        ...
        \end{gather*}
        \caption{Typing data constructs of $Core_{\Delta}$.}
    ...
    Working with existential lifetimes requires special care in the $Match$ rule.
...
%    Otherwise, this situation can be solved by introducing bivariant projection, but we dedicate this to the future work.
    ...
    We provide a prototype implementation of the type-checker, developed in Haskell\footnote{\url{https://github.com/penguin-boxx/existential-escape-analysis-paper}}.
    ...
    However, the minimum of lifetimes can be achieved only indirectly through additional lifetime constraints or by making all corresponding lifetimes equal.
    ...
    Modalities here are not first-class type constructors, but rather properties of bindings, so types with modalities cannot, for example, instantiate type variables, which limits the expressiveness further.
    ...
    Xie et al.~\cite{xie2022first} adopt higher-rank types to enforce the non-leakage of lexical effect handlers.
    ...
    Specifically, our approach incorporates the minimum operation on lifetimes and streamlines the handling of compound data types by approximating their lifetimes as the minimum of the lifetimes of their constituent components.
