...
%        National Research University Higher School of Economics, Saint Petersburg, Russian Federation
    ...
    \textbf{Abstract.}
    Programming languages frequently utilize escape analysis techniques to ensure safety of compiler optimizations and various language features.
    To achieve high precision and efficiency, particularly in interprocedural scenario, escape analysis is often integrated with the type system.
    However, such analysis tend to be rigid and usually impose excessive annotation requirements on programmers.

    In this paper, we introduce a novel type-based escape analysis approach that incorporates a minimum (union) operation on lifetimes, enabling user-provided type annotations accurately capture a function’s data flow, thereby making such annotations simpler and more intuitive to write.
...
%    Presence of these labels in type signatures makes the analysis local
    ...
    Escape analysis is often used to check if an object outlives corresponding lexical scope.
    It is commonly used by optimizing compilers to allocate some objects on the stack instead of the heap, thereby reducing garbage collection overhead and improving overall program performance~\cite{blanchet1999escape}.
    ...
    Our work is mainly motivated by application of escape analysis for safety enforcement of contextual polymorphism in capability-based effect systems~\cite{brachthauser2020effects, boruch2023capturing}.

    Integration of escape analysis with type systems give several important advantages over more classical approaches.
    Firstly, information about escaping is explicated in functions signatures, so interprocedural analysis can be implemented as intraprocedural that just consults with signatures of called functions.
    Secondly, such analysis do not produce long-distance constraints, which contributes to better error reporting.

    However, only few languages adopt escape analysis integrated with the type system because of the inherent rigidity or complexity of the resulting type systems.
    ...
    Specifically, our approach incorporates union types, which make type signatures transparently describe a function's dataflow.
    We introduce existential lifetimes to automate the handling of data type lifetimes.
    Additionally, we assume lifetimes of data types are always covariant, while still preserving safety guarantees.
    ...
    To differentiate between tracked and non-tracked types, each type \lstinline[language=colang]|T| is annotated with a special label \lstinline[language=colang]|lab|, referred to as a \textit{lifetime}: \lstinline[language=colang]{T'lab}; or \lstinline[language=colang]{(A)'lab -> B} for function types.

    The \lstinline[language=colang]|free| lifetime label designates non-tracked values.
    In contrast, the \lstinline[language=colang]|local| label appears in tracked types and is treated specially by the type system to prevent their escape from the corresponding lexical scope.
    Namely, tracked values cannot be returned from functions.
    Thus, we can specify in type of a higher-order function that it does not leak an argument function.
    ...
    For instance, the \lstinline[language=colang]|lazyMap| function does not eagerly compute and return the final result; instead, it immediately yields a collection that encapsulates the processing logic.
    ...
    Thus, such function continues to support contextual polymorphism, as it can only permit \texttt{f} to escape with the result, and the call site is made aware that the result must not escape its prescribed context:
    \begin{lstlisting}[language=colang]
    ...
    Moreover, for simplicity, all type constructors are covariant with respect to their lifetimes, which requires special care in pattern-matching rule.
    ...
    As a result, these type parameters may be instantiated with tracked types:%
    ...
    We will use $+$ on lifetimes in prefix position when taking the minimum of a bunch of lifetimes\footnote{Effectively, the empty list of lifetimes in the union denotes \lstinline[language=colang]{free} lifetime. We discuss normalized presentation later in Section\ \ref{sec:implementation}.} (like $+\overline{\Delta}$) and in infix position for the given lifetimes (like $\Delta_1 + \Delta_2$).
...
%    Otherwise, this situation can be solved by introducing bivariant projection, but we dedicate this to the future work.
    ...
    We provide the type-checker prototype implementation written in Haskell\footnote{\url{https://0x0.st/K9p6.zip}}.
    ...
    Also, we provided formalization and the type-checker prototype implementation.
