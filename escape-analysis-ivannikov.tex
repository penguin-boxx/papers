...
%                \\
    ...
    The operation $lt_\Gamma(\cdot)$ collects all lifetimes assuming typing context $\Gamma$.
...
%    fun(k) k(File())  : ((File'local) -> Unit) -> Unit
%    fun(k) k(file)    : ((File'local) -> Unit)'local -> Unit
    % todo negative are not needed
    ...
    So, the rule determines the lifetime associated with a function type as the minimum of the lifetimes of all captured variables $\overline{s}$, excluding those that are present within the function type by themselves. % todo example

    % todo it's ok to lend local things to argument functions ((File'local) -> r) -> r
    ...
    So the lifetime parameters of a data constructor do not present directly in the resulting type, which makes them similar to existential types. % todo cite
