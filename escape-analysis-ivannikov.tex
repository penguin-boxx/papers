...
\begin{document}
    ...
    % todo problem by example

%    Usually, compound data types are problematic for such approaches as there are situations in which they require types to be polymorphic over all lifetimes of components.
%    For example, consider the following Rust~\cite{matsakis2014rust} structure that encapsulates references to data sources:
...
%    \end{lstlisting}
    ...
    % todo it is an interesting problem

    % todo it is unsolved problem
